				...
				ZipCode &UnitTest_Zip)
{
	...
	// ----------------------------------------------------
		...
		std::set< ZipCode, std::function<bool(const ZipCode &)> >::iterator iter = zippy.begin();  // <-- REPLACE this LINE... shuts up warnings, its 100% wrong
		while (iter != zippy.end())
		{
			++iter;
		}
	...
	// ----------------------------------------------------
		...
		// ADD CODE HERE:
		std::set< ZipCode >::iterator zippy_it;
	...
	// ----------------------------------------------------
		// ADD CODE HERE:
		//zippy_it = std::find_if(zippy.begin(), zippy.end(), [](const ZipCode &z) { return z.custom(z); });
		zippy_it = std::find_if(zippy.begin(), zippy.end(), ZipCode::custom);
