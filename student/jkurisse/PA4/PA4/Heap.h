...
public:
	...
	void privSubUsedStats(const Used *pUsed);				//Sub to count of Useds

	void privAddCoalesceFreeStats(Free *pFree);
	void privAddCoalesceFreeStats();

	void removeFree(const Free *pFree);					//Removing a Free node from Free list
	void removeUsed(const Used *pUsed);					//Removing a Used node from Used list

	void updateFree(Free *pFree);					//Used when subdividing a used block into a new free
	void worstCaseFree(Free *pFree);				//If no adjacent blocks iterate through list
