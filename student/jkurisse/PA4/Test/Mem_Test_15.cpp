...
TEST(Mem15_combo_2, TestConfig::Flag::ALL)
{
	...
	// ----  GENERAL CHECK ------------------------------------------------------
	{
		...
		CHECK_EQUAL(GET_ALLOC_SIZE(used->mData), 0x100);
		...
		CHECK_EQUAL(GET_ALLOC_SIZE(used->mData), 0x10);
		...
		CHECK_EQUAL(GET_ALLOC_SIZE(free->mData), 0xc294);
		...
		CHECK_EQUAL(PTR_FIX(blkStart), PTR_FIX(hdrEnd));
		CHECK_EQUAL(PTR_FIX(blkEnd), 0xc800);
		CHECK_EQUAL(GET_ALLOC_SIZE(free->mData), 0xc4a0);
